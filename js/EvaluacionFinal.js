...
$(document).ready(function() {
	...
	//***************SE CREA EL TABLEBLO MEDIANTE UNA MATRIS*************************** 
	for(i = 1; i<= 9; i++) {
		x = (i % 2 == 0) ? 1 : 0;
		$("#contenedor").append("<div id=row-" + i + " class='fila'></div>")
		for(y = 1; y <= 8; y++) {
			if(y % 2 == x) {
				col_id = 'row-' + i + '-col-' + y;
	...
	for(i = 1; i<= 9; i++) {
		for(y = 1; y <= 8; y++) {
			...
			$("#" + col_id).droppable({
				...
				//****************FUNCION PARA MOVER LA FICHA LA FICHAS HACIA LAS CASILLAS VERTIVALES
				...
				//la funcion del marco donde se colocara la ficha 
				accept: function(fich) {
					var parentDiv = fich.parent().prop('id');
